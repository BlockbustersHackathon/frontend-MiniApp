...
export default function CreateCampaign({ onSubmit }: CreateCampaignProps) {
  ...
  const handleInputChange = (field: keyof CampaignFormData, value: string | number | File | null) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const validateForm = (): string[] => {
    ...
    if (!formData.projectIntroduction.trim()) errors.push('Project introduction is required');
    ...
    if (fundingGoalNum > 10000000) errors.push('Maximum funding goal is 10M USDC');
    
    if (formData.projectMode === 'launchpad') {
      if (!formData.tokenName.trim()) errors.push('Token name is required for Launchpad mode');
      if (!formData.tokenSymbol.trim()) errors.push('Token symbol is required for Launchpad mode');
    }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const selectedDate = new Date(formData.deadline);
      const now = new Date();
      const durationSeconds = BigInt(Math.floor((selectedDate.getTime() - now.getTime()) / 1000));
      ...
      await createCampaign({
        ...
        creatorReservePercentage: BigInt(CAMPAIGN_CONSTANTS.CREATOR_RESERVE_PERCENTAGE),
        liquidityPercentage: BigInt(formData.liquidityPoolPercentage),
        tokenName: formData.tokenName || formData.projectName,
        tokenSymbol: formData.tokenSymbol || formData.projectName.substring(0, 4).toUpperCase(),
      });
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {!isConnected && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
              <p className="text-yellow-800 text-sm">Please connect your wallet to create a campaign</p>
            </div>
          )}
          ...
          {(isPending || isConfirming || isConfirmed || error) && (
            <div className={`border rounded-lg p-4 ${
              ...
              {isPending && <p className="text-blue-800 text-sm">Confirm transaction in your wallet...</p>}
              {isConfirming && <p className="text-blue-800 text-sm">Transaction pending...</p>}
              {isConfirmed && (
          ...
          <div className="space-y-3">
            <label className="block text-sm font-medium text-gray-700">project mode</label>
          ...
          {/* Upload Project Icon */}
          <div className="space-y-3">
            <label className="block text-sm font-medium text-gray-700">upload project icon</label>
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-gray-400 transition-colors">
              ...
              <label htmlFor="projectIcon" className="cursor-pointer">
                {formData.projectIcon ? (
                  <span className="text-sm text-gray-600">{formData.projectIcon.name}</span>
                ) : (
          ...
          {formData.projectMode === 'launchpad' && (
            <>
              ...
              {/* Keep Token */}
              <div className="space-y-3">
                <label className="block text-sm font-medium text-gray-700">keep token</label>
                <input
                  type="number"
                  value={formData.keepToken}
                  onChange={(e) => handleInputChange('keepToken', e.target.value)}
                  placeholder="100"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  required
                />
              ...
              <div className="space-y-3">
                <label className="block text-sm font-medium text-gray-700">liquidity pool parameters</label>
                <div className="flex items-center space-x-3">
                  <div className="flex-1 h-2 bg-gray-200 rounded-full">
                    <div 
                      className="h-full bg-purple-600 rounded-full transition-all duration-300"
                      style={{ width: `${formData.liquidityPoolPercentage}%` }}
                    ></div>
                  </div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('liquidityPoolPercentage', parseInt(e.target.value))}
